...
public enum Catalog {
    public static let arguments: [Operation:[String]] = [
        ...
        .readCSV:["path","output"], .appendCSV:["path","columns","values"], .transformTable:["table","action","column","value","output"], .writeCSV:["table","path"], .resizeImage:["source","destination","width","height"], .convertImage:["source","destination","format"], .driveTime:["origin","destination","output"], .createFolder:["path"],
    ...
    public static var instructions: String {
        "Return only steps from this catalog. Parameters are an object of string values. Fill the keys listed for that operation; set every other schema-required key to the empty string. Give every step a unique id. Use {{variable}} or {{row.Column}} references. Built-in variables: {{file}} (full path of the file that started the routine, or of the table the values were copied from), {{folder}} (its folder), {{stem}} (its name without extension), {{filename}}, {{ext}}, {{today}} (yyyy-MM-dd), {{home}} (the user's home folder). Values read by readText/readURL/copyText are stored under their output name and referenced as {{output}}. JSON arrays for columns/values. forEach source is a named table (from readCSV output) or a JSON list, item is a variable name, followed by body and endLoop. ifMatches uses a regular expression and ends with endIf. Targets require the exact app bundle id (for example com.apple.Safari) plus role (AXTextField, AXButton, AXStaticText, AXLink…) and the visible label or identifier exactly as observed, plus optional ancestor names. Never invent paths, mappings, selectors, or transformations that the evidence does not show; use ask for missing information. Never return executable code. Table steps read and write CSV files; readCSV path is the observed table path, written as {{file}} when that table is the routine's input; writeCSV must write to a new path such as {{folder}}/{{stem}}-clean.csv. CSV transform actions: \(transformActions.joined(separator:", ")). split value is JSON {separator,columns}; formatDates value is JSON {from,to}; renameColumn value is new name; filter value is regex; template value is a {{row.Column}} template; regexExtract value is regex; sort value is ascending/descending; keepColumns/dropColumns value is a JSON array of names. skipRows value is an integer count of leading data rows to skip. moveFile/renameFile/copyFile take absolute source and destination paths built from variables; resizeImage takes width and height in pixels (aspect ratio is preserved) and convertImage takes format png, jpg or tiff; a destination must be a new file. appendCSV creates the CSV when missing; columns and values are JSON arrays of equal length. driveTime looks up the real driving time in minutes from origin to destination (street addresses or place names, the same ones the user pasted into the maps site) and stores the whole minutes under output; use it instead of driving a maps website, with the origin the user always used as a constant and destination {{row.Address}} (the table column holding the address) inside a forEach over the readCSV table; then appendCSV the address and {{minutes}} to {{folder}}/{{stem}}-drive-times.csv so the results are kept. createFolder creates one folder at an absolute path (parents included, existing folders are left alone); when the user made folders named after copied table cells, loop over the table with forEach and use <the Finder folder shown in the evidence>/{{row.first}} ({{row.first}} is the first column of the current row; {{row.Column}} names a specific column). Triggers: manual uses empty value and app; context or loop uses the app bundle id and value is an https/http URL prefix; file uses value as an absolute folder path (no wildcards), app as file extension without dot; schedule value is HH:mm. Catalog: " + Operation.allCases.map { "\($0.rawValue)(\((arguments[$0] ?? []).joined(separator:",")))" }.joined(separator:"; ")
    }
