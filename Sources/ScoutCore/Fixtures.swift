...
public enum Fixtures {
    ...
    public static func evidence(_ shape: String, root: String = "/tmp/RoutineScoutDemo", now: Date = Date()) -> [Evidence] {
        ...
        for run in 0..<3 {
            ...
            case "travel":
                ...
                e("com.apple.Safari","click","Directions",["url":"https://www.google.com/maps/"],1)
                e("com.apple.Safari","paste","Choose destination, or click on the map...",["label":"Choose destination, or click on the map...","value":addresses[run],"url":"https://www.google.com/maps/dir/\(office)/"],2)
                e("com.apple.iWork.Numbers","click","Drive Time (min)",["row":"B\(run+2)","value":minutes[run],"document":"addresses.csv","path":"\(root)/addresses.csv"],10,role:"AXCell")
            case "transform":
    ...
    public static func driveTimes(root: String) -> Automation {
        var a = Automation(name:"Drive times from the office",description:"Look up the real driving time from the office to every address in the table and save a copy of the table with the Drive Time column filled in.",steps:[
            Step(.readCSV,"Read the addresses",["path":"{{file}}","output":"addresses"]),
            Step(.forEach,"For each address",["source":"addresses","item":"row"]),
            Step(.driveTime,"Look up the drive time from the office",["origin":office,"destination":"{{row.Address}}","output":"minutes"]),
            Step(.appendCSV,"Write the minutes next to the address",["path":"{{folder}}/{{stem}}-drive-times.csv","columns":"[\"Address\",\"Drive Time (min)\"]","values":"[\"{{row.Address}}\",\"{{minutes}}\"]"]),
            Step(.endLoop,"Next address")
        ])
        a.inputs = [Parameter("file",root+"/addresses.csv")]
        a.suggestedTriggers = [RunTrigger(),RunTrigger("file",title:"When an address list arrives in this folder",value:root,app:"csv")]
        return a
    }
